...
PaddingStrategy.registry["zero_padding"] = ZeroPadding
...
def get_padding(strategy_name: str) -> PaddingStrategy:
    ...
    if cls is None:
        raise ValueError("Unknown padding strategy: {}".format(strategy_name))
    return cls()
...
SamplingStrategy.registry["farthest"] = FarthestPointSampling
...
def get_sampler(strategy_name: str) -> SamplingStrategy:
    ...
    if cls is None:
        raise ValueError("Unknown sampling strategy: {}".format(strategy_name))
    ...
    if show and has_outliers:
        ...
        for i, (pc, mask) in enumerate(zip(pcl_seq, outlier_masks)):
            ...
            ax.scatter(normal_points[:,0], normal_points[:,1], normal_points[:,2],
                       color=cmap(i % 10), s=20, label="Frame {} normal".format(i))
            # Outlier points
            outlier_points = xyz[mask]
            if len(outlier_points) > 0:
                ax.scatter(outlier_points[:,0], outlier_points[:,1], outlier_points[:,2],
                           color='red', s=50, label="Frame {} outlier".format(i))
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title("Sequence True Outliers (red) - Factor {}".format(factor))
        ax.legend()
        plt.show()
